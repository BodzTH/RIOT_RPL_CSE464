<<<<<<< HEAD
# 📡 RPL Network Analysis: Convergence & Loop Detection
### IoT Security Project - Members 6, 7, and 8

## 📖 Project Overview
This module focuses on analyzing the stability and recovery speed of the RPL routing protocol under stress. We performed automated failure tests on specific nodes to measure **Convergence Time** and ensure **Routing Loop Freedom**.

---

## 🛠️ Member 06: Automation Scripting
**Objective:** Automate the process of killing nodes and capturing traffic.
- **Tools:** Bash Scripting, `tshark`.
- **Methodology:** - Created a script to continuously monitor the network.
  - Triggered node failures (simulated death).
  - Automatically captured packets during the "Chaos" interval using `tshark`.
- **File:** `convergence_script.sh` (Included in this folder).

---

## ⏱️ Member 07: Convergence Analysis
**Objective:** Calculate the time taken for a child node to find a new parent.
- **Metric:** Time difference between `DODAG Information Solicitation (DIS)` and the first `DAO-ACK` from the new parent.
- **Results:**
  - Analyzed 5 different scenarios (Nodes 1, 4, 5, 6, 7).
  - Verified network recovery speed and stability.
- **Data:** See `convergence_results` files / `pcap` captures.

---

## 🕵️‍♂️ Member 08: Loop Hunter
**Objective:** Detect routing loops during the network recovery phase.
- **Inspection Criteria:**
  1. **TTL/Hop Limit:** Monitored for packets with decreasing Hop Limit (`ipv6.hop_limit < 5`).
  2. **Rank Property:** Verified that RPL Rank strictly decreases towards the root (no "Count-to-Infinity").
  3. **Errors:** Checked for ICMPv6 "Destination Unreachable" messages.
- **Verdict:** **Clean Health**. No routing loops were detected in any of the stress test scenarios.

---
### 📂 Included Files
- `convergence_script.sh`: The Bash script used for automation.
- `*.pcap`: Packet capture files of convergence events.
- `Health_Report`: Analysis of routing loops and network stability.
=======
...
>>>>>>> 80f78abad27d48ed4cb0ee77fbb7c0cdf9ffac87
